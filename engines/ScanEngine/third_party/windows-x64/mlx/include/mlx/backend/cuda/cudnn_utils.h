...
class DnnGraph : public fe::graph::Graph {
 public:
  ...
  fe::error_t prepare();
  // Skip heuristics and pin a single official engine+knobs.
  fe::error_t prepare_pinned(
      int64_t engine_id,
  ...
  fe::error_t encode_capturing(
      cu::CommandEncoder& encoder,
      std::unordered_map<int64_t, void*> variant_pack);
  // Run on the encoder stream without capturing. TF32 tensor-core engines
  // often fail cudaGraphAddKernelNode; official torch also launches them
  // eagerly on the cuDNN handle stream.
  fe::error_t encode_eager(
      cu::CommandEncoder& encoder,
      std::unordered_map<int64_t, void*> variant_pack);
