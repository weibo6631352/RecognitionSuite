...
class CommandEncoder {
 public:
  ...
  bool needs_commit();
  // Launch the pending CUDA graph but keep temporaries alive. Used so an
  // eager cuDNN conv can run after recorded NHWC↔NCHW copies without
  // freeing this op's buffers (commit() would drop them).
  void flush_graph();
  void commit();
