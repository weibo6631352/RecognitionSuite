...
3. UI 验收验证：加载冻结测试集清单，批量执行扫描或语音识别，在界面内计算并判定 98% 指标，同时展示未经校准的中间置信度。
...
验收清单格式、完整性规则和证据输出见 [`docs/验收验证.md`](../../docs/验收验证.md)。任何样本失败或结果缺失都会使 UI 结论变为“验证无效”；置信度属于未经校准的辅助指标，不能替代与真值比较得到的准确率。
